...
import de.abring.gui.imageOCR.ImageOCR;
 ...
 * @author Karima
 */
public class JPictureFrameOCR extends javax.swing.JFrame {
     ...
     * @param file
     */
    public JPictureFrameOCR(File file) {
        initComponents();
        loadFile(file);
    }
    ...
    private void loadFile(File file) {
        try {
            ...
            if (cells[page] == null) {
                cells[page] = loadPage(page);
            }
            showPage(cells[page].getMaskedImage());
            changeBtnState();
        } catch (IOException ex) {
            Logger.getLogger(JPictureFrameOCR.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private Cell loadPage(int page) {
...
//        readCell(cell);
    ...
    private void readCell (Cell cell) {
        for (Cell child : cell.getChildList()) {
            System.out.println("------------------------------------------------------------------------------------------------------------------------------------");
            for (Cell part : child.getPartsList()) {
                imageToString(part.getImage());
            }
        }
    }
    
    private String imageToString (BufferedImage image) {
        ImageOCR imageOCR = new ImageOCR(this, true, image);
        imageOCR.requestFocus();
        imageOCR.setVisible(true);
        return imageOCR.getString();
    }
    
    private void showPage(BufferedImage image) {
